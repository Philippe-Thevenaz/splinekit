...
def spline_polynomial (
    *,
    ...
    The returned polynomial ``polyQQ`` is valid over the ``sympy`` domain
    ``"QQ"``. If it is desired to evaluate it at floating-point arguments,
    it may be convenient to perform the conversion
    ``polyRR = sympy.Poly.from_poly(polyQQ, domain = "RR")``.
