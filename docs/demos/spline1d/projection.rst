...
where :math:`f` is the spline :math:`f:{\mathbb{R}}\rightarrow{\mathbb{R}},x\mapsto f(x)=\sum_{k\in{\mathbb{Z}}}\,c[{k\bmod K}]\,\beta^{n}(x-\delta x-k)` with same period :math:`K` but with desired arbitrary degree :math:`n\in{\mathbb{N}}` and delay :math:`\delta x\in{\mathbb{R}}.`
...
..  math::
    c[k]=\sum_{p\in{\mathbb{Z}}}\,\left(\sum_{q\in{\mathbb{Z}}}\,\left(b^{2\,n+1}\right)^{-1}[q]\,c_{0}[{\left(p-q\right)\bmod K}]\right)\,\beta^{n+n_{0}+1}(\left(k-\delta x_{0}+\delta x\right)-p),
...
..  admonition:: Jupyter Lab notebook
    ...
    `Projection of a spline <https://splinekit.github.io/splinekit-jupyterlite/lab/?path=periodic-spline/extrema/spline_projection.ipynb&mode=single-document>`_